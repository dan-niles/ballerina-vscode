...
export const AWAITING_INPUT_LABEL = "WSO2 Agent Builder Intelligence needs your input";

export function activeStateLabel(status: AgentRunStatus): string {
    switch (status.state) {
        case "completed":
            return status.aiPanelOpen ? "Done" : "Done — click to open WSO2 Agent Builder Intelligence";
        case "running":
            return status.label ?? "Working on it…";
        case "awaiting-input":
            return status.label ?? AWAITING_INPUT_LABEL;
        case "error":
            return status.label ?? "WSO2 Agent Builder Intelligence hit an error";
        default:
